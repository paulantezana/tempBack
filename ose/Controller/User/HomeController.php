...
EOD;
    ...
    function sign($msg, $key, $alg = 'SHA256')
    {
        if ($alg != 'SHA256') {
...
//        $timestamp = is_null(static::$timestamp) ? time() : static::$timestamp;
        ...
        // Check the signature
        if (!verify($bodyb64, $sig, $key, 'SHA256')) {
...
//        }
        ...
        $llave = encode($data,$privateKey);

        $dataRecuperado = decode($llave,$publicKey);

        var_dump($llave);
        var_dump($dataRecuperado);
