...
<div class="container pt-5 pb-5">
    <?php if ($parameter['message'] ?? '') :
        $type = $parameter['messageType'] ?? '';
        $className = $type === 'error' ? 'danger' : ($type === 'success' ? 'success' : '');
        ?>
        <div class="alert alert-<?= $className ?> mt-5" role="alert">
            <?= $parameter['message'] ?? '' ?>
        </div>
    <?php endif; ?>
        ...
        <div class="col-auto d-flex align-items-center">
            <div class="dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" data-toggle="modal" data-target="#summaryGeneralModal" href="#">Proceso automático general</a>
                    <a class="dropdown-item" data-toggle="modal" data-target="#summaryCustomerModal" href="#">Proceso automático por cliente</a>
                    <a class="dropdown-item" data-toggle="modal" data-target="#summaryCustomModal" href="#">Resumen personalizado</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <div class="collapse" id="collapseAdvancedOptions">
                <form action="<?= FOLDER_NAME . '/Summary'?>" method="GET" class="mt-4">
                    <div class="form-row">
                        <div class="form-group col-lg-3">
                            <label for="filterReferenceUserId">Usuario</label>
                            <select class="custom-select" name="filter[referenceUserId]"  id="filterReferenceUserId">
                                <?php  foreach ($parameter['user'] ?? [] as $row): ?>
                                    <option
                                        value="<?= $row['id_user'] ?>"
                                        <?php echo ($parameter['filter']['referenceUserId'] ?? '') == $row['id_user'] ? 'selected' : '' ?>
                                    >
                                        <?= $row['names'] ?>
                                    </option>
                                <?php endforeach; ?>
                    ...
                    <div class="btn-group">
                        ...
                        <a href="<?= FOLDER_NAME . '/Summary'?>" class="btn btn-light">Mostrar Todo</a>
                    </div>
                </form>
            </div>
        ...
        <div class="card-body">
            <div class="breadcrumb-line breadcrumb-line-component mb-2">
                <ul class="breadcrumb bg-light">
                    <li><img style="max-width: 30px;" src="<?= FOLDER_NAME . '/Asset/Images/sunatLogo.png'?>" class="position-left"><span class="text-semibold">Estados SUNAT: </span></li>
            ...
            <?php
                $whitFilter = '';
                if (($parameter['filter']['referenceUserId'] ?? '')){
                    $whitFilter .= '&filter%5BreferenceUserId%5D=' . $parameter['filter']['referenceUserId'];
                }
            ?>
            <nav aria-label="...">
                <ul class="pagination">
                    <li class="page-item <?php if(($parameter['summary']['current'] ?? 0) <= 1){ echo 'disabled'; } ?>">
                        <a class="page-link" href="<?php
                            if(($parameter['summary']['current'] ?? 0) <= 1){
                                echo '#';
                            } else {
                                echo "?page=".(($parameter['summary']['current'] ?? 0) - 1) . $whitFilter;
                            }?>"
                           tabindex="-1"
                           aria-disabled="true"
                        >
                    ...
                    <?php for ($i=1; $i <= $parameter['summary']['pages'] ?? 1; $i++): if ($i == $parameter['summary']['current'] ?? 1): ?>
                        <li class="page-item active" aria-current="page">
                            <a class="page-link" href="?page=<?php echo $i . $whitFilter ?>"><?= $i ?> <span class="sr-only">(current)</span></a>
                        </li>
                    <?php else:  ?>
                        <li class="page-item"><a class="page-link" href="?page=<?php echo $i . $whitFilter ?>"><?= $i ?></a></li>
                    <?php endif; endfor; ?>
                    <li class="page-item <?php if(($parameter['summary']['current'] ?? 0) >= ($parameter['summary']['pages'] ?? 0)){ echo 'disabled'; } ?>">
                        <a class="page-link" href="<?php
                            if(($parameter['summary']['current'] ?? 0) >= ($parameter['summary']['pages'] ?? 0)){
                                echo '#';
                            } else {
                                echo "?page=".(($parameter['summary']['current'] ?? 0) + 1) . $whitFilter;
                            } ?>">
...
<script type="text/javascript" src="<?= FOLDER_NAME ?>/Asset/Js/Manager/Summary.js"></script>
